...
int main(int argc, char *argv[])
{
    ...
    // 【核心改造 1】：使用 Lambda 表达式实现数据驱动 (Event-Driven)
    // 只要一收到 Odom，立刻更新数据并触发 fsm.process()，实现绝对的零延迟下发！
    ros::Subscriber odom_sub = 
        nh.subscribe<nav_msgs::Odometry>("/mavros/local_position/odom", 
                                         5,
                                         [&fsm](const nav_msgs::OdometryConstPtr& msg) {
                                        fsm.odom_data.feed(msg); // 1. 先更新状态机里的 Odom 数据
                                        fsm.process();           // 2. 立刻运算并把指令发给 PX4
                                        },
                                        ros::VoidConstPtr(),
                                        ros::TransportHints().tcpNoDelay());
    ...
    fsm.local_ctrl_pub     = nh.advertise<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/local", 1);
...
//////////////
    ...
    ROS_INFO("[PX4CTRL] PX4 Connected! Starting Event-Driven Control Loop.");

    // 【核心改造 2】：删除了原本的 ros::Rate r 和 while(ros::ok()) 死循环
    // 交给纯粹的 ros::spin() 去阻塞监听回调。有 Odom 来就解算，没 Odom 来就安静等待。
    ros::spin();
