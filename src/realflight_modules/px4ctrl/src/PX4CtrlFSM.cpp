...
void PX4CtrlFSM::process()
{
	...
	case AUTO_TAKEOFF:
	{
		...
		{
            // 同样加入减速逻辑：确保上升速度降下来后再切悬停
            if (std::abs(odom_data.v(2)) > 0.15) {
                des = get_takeoff_land_des();
                des.v.z() = 0.0; // 强制要求 Z 轴速度为 0 以刹车
            } else {
			    state = AUTO_HOVER;
			    set_hov_with_odom();
	...
	case RC_CTRL:
	{
		...
		{
            // 【你的绝佳思路】：摇杆回中时，等待速度降到0.15以内再切入悬停
            if (odom_data.v.head<2>().norm() > 0.15) 
            {
                // 此时保持在 RC_CTRL，但因为摇杆已回中，get_rc_des() 必然输出 v=0
                // 下游控制器会自动让飞机刹车
                des = get_rc_des(); 
            }
            else 
	...
	case CMD_CTRL:
	{
		if (!cmd_is_received(now_time))
		{
            // 【你的绝佳思路】：脱离指令后，先强行减速到0.15以内再切入悬停
            if (odom_data.v.head<2>().norm() > 0.15) 
            {
                des.p = odom_data.p;
                des.v = Eigen::Vector3d::Zero(); // 强制目标速度为 0
...
Desired_State_t PX4CtrlFSM::get_rc_des()
{
	...
	des.v = Eigen::Vector3d(4.0 * rc_data.ch[1] * (param.rc_reverse.pitch    ? 1 : -1), 
							4.0 * rc_data.ch[0] * (param.rc_reverse.roll     ? 1 : -1), 
							3.0 * rc_data.ch[2] * (param.rc_reverse.throttle ? 1 : -1));
	des.a = Eigen::Vector3d::Zero();
	des.j = Eigen::Vector3d::Zero();
	des.yaw = 0.0;
